...
public class CbsCashDepositServiceSoapAdapter implements CbsTransactionServiceAdapter<CbsCashDepositRequest, CbsBaseResponse> {
	
	private static final String  DEPOSIT_PRODUCT = "CHDP";
	private static final String  DEPOSIT_TXNTRAN = "CHD";
	@Override
	public CbsBaseResponse performTransaction(CbsCashDepositRequest p_tranRequest)
		...
		LoggerUtil.cbsJobDebug("Deposit Trans started");
		try {
			FCUBSRTService l_service = new FCUBSRTService();		
			FCUBSRTServiceSEI l_serviceSEI = l_service.getFCUBSRTServiceSEI();
			
			CREATETRANSACTIONIOPKREQ l_cbsReq = cashDepositTranRequest(p_tranRequest);
			...
			LoggerUtil.cbsJobDebug("Deposit Trans Ended");
			return l_res;
		} catch(Exception e){
			l_res.setStatus(false);
			l_res.setResponseCode("MFI10101");
			l_res.setResponseMessage("Unhandeled Exception while process deposit request.");
			LoggerUtil.cbsJobError("Unhandeled Exception :", e);
			return l_res;
		}*/
	}
	
	/*private CREATETRANSACTIONIOPKREQ cashDepositTranRequest(CbsCashDepositRequest p_tranRequest) {
		...
		transactionDetails.setPRD(DEPOSIT_PRODUCT);
		...
		transactionDetails.setTXNTRN(DEPOSIT_TXNTRAN);
